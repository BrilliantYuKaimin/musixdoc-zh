\chapter{\musixtex 的简介}

本章不是 \musixtex 的使用教程，而是对它的一些功能、怪癖和历史的概述。

\musixtex 是一组能在 \TeX 系统中进行音乐排版的宏和字体。需要先安装 \TeX 它才能工作。
...
\musixtex 也可以处理歌词。There is a set of primitive
...
\LaTeX 是一组简化文档排版的宏，大多数 \TeX 用户也熟悉 \LaTeX。事实上很多人只使用 \LaTeX。
...
\section{\musixtex 的主要特点}
 \subsection{音乐排版是二维的} Written music is not
...
\section{一个简单示例}
在讲解更多细节之前，we give below an example of the two first
...
text-placement macros defined in \verb|musixlyr.tex| （见第\pageref{lyrics}页第\ref{lyrics}章）。
 \item
Between \verb|\startpiece| and \verb|\stoppiece| or \verb|\endpiece|,
...
\section{\musixtex 的安装和使用}\label{using}
...
自 2003 年陶平博士去世以来，\musixtex 的维护者阵容由奥利维尔·沃格尔、森本博明、鲍勃·坦南特、
安德烈·范·里克格姆、科尔内利乌斯·诺亚克和唐·西蒙斯等人更替。
